...
export default function PantallaForo() {
  ...
  const forumIdStr = Array.isArray(params.forumId)
    ? params.forumId[0]
    : (params.forumId as string);
  const nombre = Array.isArray(params.nombre)
    ? params.nombre[0]
    : (params.nombre as string) || "Foro";
  ...
  // Carga las discusiones del foro desde Moodle
  const cargar = async () => {
    try {
      setLoading(true);
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: "/posts",
                  params: {
                    discussionId: String(item.discussion),
          ...
          ListEmptyComponent={
            <Text style={styles.vacio}>No hay discusiones visibles.</Text>
          }
        />
      )}
    </View>
